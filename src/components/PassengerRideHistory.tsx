...
function PassengerRideHistory() {
  const [rides, setRides] = useState<PassengerRideHistoryType[]>([]);
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-1">
              View all completed motorcycle rides you took as a Passenger.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 bg-emerald-50 text-emerald-800 px-4 py-2 rounded-2xl border border-emerald-200 self-start sm:self-auto">
            ...
            <span className="text-sm font-extrabold">{rides.length} Trips Completed</span>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {rides.length === 0 ? (
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center">
              ...
              <h2 className="text-xl font-bold text-gray-800">No completed trips yet</h2>
              <p className="text-xs text-gray-500 mt-1 max-w-sm mx-auto">
                Once you book and complete motorcycle rides, your trip history will appear here.
              </p>
            </div>
          ) : (
            rides.map((ride) => (
              <PassengerRideHistoryCard key={ride.requestId} ride={ride} />
            ))
          )}
        </div>
      </div>
